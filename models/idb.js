...
let db;
// establish a connection to IndexedDB database called 'social_network' and set it to version 1
...
function saveRecord(record) {
    ...
    const transaction = db.transaction(['social_network'], 'readwrite');
...
function uploadPizza() {
    ...
    const pizzaObjectStore = transaction.objectStore('social_network');
        ...
        if (getAll.result.length > 0) {
            fetch('/api/pizzas', {
                ...
                .then(serverResponse => {
                    ...
                    const transaction = db.transaction(['social_network'], 'readwrite');
                    // access the social_network object store
                    const pizzaObjectStore = transaction.objectStore('social_network');
